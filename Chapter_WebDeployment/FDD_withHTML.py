...
def getLatestProcessData():
    ...
    processData = processDataAll[np.random.randint(0,np.size(processDataAll,0)),:][None,:] # pick a random row and return a 2D array with 1 row
...
def runPCAmodel():
    ...
    if (currentQ > PCAmodelData["Q_CL"]) or (currentT2 > PCAmodelData["T2_CL"]):
        processState = 1 # numeric flag to indicate presence of no fault
    else:
        processState = 0 # numeric flag to indicate no fault
...
class FDDapp(object):
    ...
    def getResults(self):
        ...
        frontEndCode = frontEnd_template.render(state=processState) # generates front-end code using the template
...
if __name__ == '__main__':
    cherrypy.quickstart(FDDapp(), config=conf) # when this script is executed, host FDDapp app 
